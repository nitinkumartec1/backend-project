  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/upload" className="bg-purple-600 px-4 py-2 rounded text-white hover:bg-purple-700">Upload New Video</Link>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border rounded">
          ...
          <tbody>
            {videos.map(video => (
              <tr key={video._id} className="border-b hover:bg-gray-50">
               <td className="py-2 px-4">
                ...
                <td className="py-2 px-4">
                  <span className={`px-2 py-1 rounded text-xs ${video.isPublished ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                ...
                <td className="py-2 px-4 text-right">{video.views}</td>
                 <td className="py-2 px-4 text-right text-sm text-gray-500">{new Date(video.createdAt).toLocaleDateString()}</td>
                <td className="py-2 px-4 text-center space-x-2">
                  <button onClick={() => handleDelete(video._id)} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 text-sm">Delete</button>
            ...
            {videos.length === 0 && (
                <tr>
                    <td colSpan="7" className="text-center py-4 text-gray-500">No videos uploaded yet.</td>
                </tr>
            )}
          </tbody>
        </table>
